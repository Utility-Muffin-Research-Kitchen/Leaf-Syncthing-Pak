...
func EnsureIdentity(ctx context.Context, options IdentityOptions, recovery RecoveryResult) (Identity, error) {
	...
	case RecoveryReady:
		identity, err := validateGeneratedIdentity(ctx, options.Binary, options.ConfigDir, options.DataDir, options.UpstreamVersion, options.GUISocket)
		if err != nil {
			return Identity{}, err
		}
		return identity, nil
...
func validateGeneratedIdentity(ctx context.Context, binary, configDir, dataDir, upstreamVersion, guiSocket string) (Identity, error) {
	...
	identity, current, err := inspectGeneratedIdentity(ctx, binary, configDir, dataDir, upstreamVersion, guiSocket)
	if err != nil {
		return Identity{}, err
	}
